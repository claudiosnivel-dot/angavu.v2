...
| **Ultimo aggiornamento** | 2026-08-23 (**sessione di BUILD rifinitura HIG — R-02**: titolo unico per schermata, eliminato il doppione nav-large + hero in-body a gradiente su 5 schermate. **CI Apple run #39 `success`** (`091b71f`). Piano build 11/11 + `wiring` 8/8 + guscio UI 8/8 invariati; rifinitura HIG **3/12** (R-00, R-01, R-02 chiusi)) |
| **Sessione corrente** | **Chiusa sul verde (BUILD rifinitura HIG — R-02).** **R-02 — titolo unico per schermata (trasversale)**: ogni schermata montava un `navigationTitle(...)` in **large title** PIÙ un header in-body a gradiente con lo **stesso testo** → la scritta appariva due volte. Portata la nav bar a `.navigationBarTitleDisplayMode(.inline)` (guardata `#if canImport(UIKit)`, cross-platform per il build macOS in CI) su **Home, Dashboard, CategoryReview, Compression, HonestReport**, replicando la convenzione **già presente e verde** in CI su `NonGoalsView`. L'hero in-body a gradiente resta l'**unica intestazione visibile**; il `navigationTitle` è mantenuto impostato (non svuotato) così VoiceOver ha il titolo di schermata e i push a valle hanno il contesto del back-button. `ExtraPhotoDomainsView` (list, nessun hero in-body) e `ThemeSettingsView` (Form) hanno **un solo** titolo ciascuna → nessun doppione → **invariate** (già conformi; svuotarle rimuoverebbe l'unica intestazione senza un hero a sostituirla). **VERDE (comando, L-COL-002)**: **CI run #39 `success`** (`091b71f`) — `swift build` (-warnings-as-errors), `swift test` (target_tests + regressione, invariata), `swiftlint lint --strict`, build app iOS. Verde al primo colpo. **Copertura (L-COL-006)**: task **solo-View**, nessuna decisione presentabile da estrarre → **nessun `target_test` nuovo**; le 5 View sono compilate dai due job CI ma **senza test di rendering** (resa a runtime — titolo non più doppio — dichiarata non coperta). **Nessuna logica nuova Domain/Data**: altitudine invariata. Baseline privacy invariata (nessun permesso/rete/framework nuovo). **Prossimo: R-03** (raggruppamento VoiceOver di righe e card + trait `isHeader` sui titoli di stato/sezione + icone decorative `accessibilityHidden`). **[Storico R-00 + R-01, chiusi a run #38]:** Costruiti i due task **alti** della coda R-* del `HIG-REFINEMENT-PLAN.md`, ognuno chiuso al confine CI. **R-00 — persistenza onboarding (bug reale)**: `App/ContentView.swift` passava `didFinishOnboarding` come `@State` → azzerato a ogni cold-launch, l'onboarding-manifesto ricompariva a ogni avvio; ora `@AppStorage(OnboardingGate.didFinishStorageKey)`, compare una sola volta per installazione. Decisione «mostrare l'onboarding?» estratta nel PURO `OnboardingGate` (Features) → oracolo `OnboardingGateTests` (3 target_test) su Linux. **R-01 — cifra-hero scalabile (trasversale)**: le cifre-hero usavano una dimensione FISSA `.font(.system(size: 44/40 …))`, non scalabile col Dynamic Type e troncata alle AX sizes, con `monospacedDigit` incoerente. Introdotto `AuroraType` (PURO): stile cifra-hero come dati — text style semantico `largeTitle`, rounded, bold, `monospacedDigit`, cap Dynamic Type `accessibility3` (oracolo `AuroraTypeTests`, 3 target_test). Resa SwiftUI in `AuroraTheme`: `Font` derivato + modificatore `.auroraHeroNumber()` (`dynamicTypeSize` capped, `.contentTransition(.numericText())` **gated su Reduce Motion**, unico punto di traduzione). Riuso su tutte le cifre-hero: Dashboard, Review categorie, Report onesto (esatta+stima), Compressione; `monospacedDigit` aggiunto anche al valore «liberabili ora» del Report onesto. **VERDE (comando, L-COL-002)**: **CI run #38 `success`** — build (-warnings-as-errors), test (target_tests+regressione), `swiftlint lint --strict`, build app iOS. NB: run #37 era rosso **solo** sul lint (nesting `AuroraType.swift:23`, `TextStyle` a 2 livelli → promosso a `AuroraType.HeroTextStyle`), fix riverificata su #38. **Copertura (L-COL-006)**: helper puri coperti dai target_test (incl. cap Dynamic Type e cifre monospaziate); le View SwiftUI compilate dai due job CI ma **senza test di rendering**. **Nessuna logica nuova Domain/Data**: altitudine invariata. Baseline privacy invariata (nessun nuovo permesso/rete/framework). Prossimo: **R-02** (titolo unico per schermata, niente doppioni nav-large + header gradiente su 6 schermate). |
...
> **⭐ IN CORSO = BUILD della rifinitura HIG (3/12).** Coda `R-00…R-11` del
...
> per schermata — nav bar `.inline` guardata `#if canImport(UIKit)` su
> Home/Dashboard/CategoryReview/Compression/HonestReport, replicando `NonGoalsView`;
> ExtraPhotoDomains/ThemeSettings invariate) a **run #39 `success`** (`091b71f`).
> **Prossimo consigliato: R-03** (raggruppamento VoiceOver di righe/card +
> `accessibilityElement(children:.combine)` con label/valori umani, trait `isHeader`
> sui titoli di stato/sezione, icone decorative `accessibilityHidden`). Poi i restanti
> medi R-04…R-09, infine i bassi R-10/R-11. Nessuna logica
...
| Ultimo commit | `091b71f` feat(ui-shell) R-02 titolo unico per schermata (nav bar inline) — **CI verde run #39**. Precedenti: `9d37fad` (docs chiusura R-00+R-01), `f0b22ec` (R-01 fix nesting, run #38 verde), `dea3fc5` (R-01 cifra-hero), `f934ad4` (R-00 onboarding `@AppStorage`) |
