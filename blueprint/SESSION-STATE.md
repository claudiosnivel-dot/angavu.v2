...
| Ultimo commit | `18c03b7` **Fase E** (flusso primo avvio "Shazam": E-1 `ScanFlowPresentation` + E-2 tasto gigante animato/carosello + E-3 schermata successo coriandoli) — **CI run #71 `success`** (entrambi i job su runner reali). Piano POST-DEVICE VERIFICATO in CI: P0-2/3/4 (#60/#61), A-1/A-2/A-3 + P0-1 + B-1 + E-4 (#66), **B-2** batch video (#67), **Fase D** cache/progresso (#69), **C-1** categorie foto (#70), **E-1/E-2/E-3** flusso Shazam (#71). **Ancora da fare**: **P0-2b** (residenza per-asset precisa ~8 GB, progettata per non rifreezare) + follow-up device-only (sink observer→PhotoKit). **Fortemente consigliato: test on-device** dell'`.ipa` aggiornato — miniature/selezione/categorie/batch video/cache **e ora il flusso Shazam** (tasto animato, battito, riempimento, carosello, coriandoli) NON sono coperti dalla CI headless (L-COL-006). CI normale (repo pubblico). |
