...
| Ultimo commit | `0ac3dfa` docs: `POST-DEVICE-UX-PLAN.md` (piano post-device, solo documentazione — no CI). Ultimo commit di CODICE: `8a383b0` letture off-main (Dashboard/Report/Review/Compressione) — **CI Apple `success`** (run id 32835018284). Sequenza di sessione: `03c49b1` (scan: mappatura O(N²)→O(N)+lint, run id 32829651215), `e3011ae` (indice SwiftData su contesto dedicato), `db71030`→`db3fefa` (upsert batch), `0f04ecc` (docs). Storico blueprint: `d4d1cac` feat(ui-shell) R-11 (CI verde run #49), `0d9ed69` (R-10, #48) |
