...
| **Ultimo aggiornamento** | 2026-08-23 (**sessione di BUILD rifinitura HIG — R-04**: etichette VoiceOver azione+oggetto sui bottoni «Fondi»/«Rimuovi» + hint «Stima il risparmio» sulla riga-candidato compressione + tap target ≥44pt + `swipeActions` distruttivo human-gated. **CI Apple run #42 `success`** (`59616f0`, verde al primo colpo). Piano build 11/11 + `wiring` 8/8 + guscio UI 8/8 invariati; rifinitura HIG **5/12** (R-00…R-04 chiusi)) |
| **Sessione corrente** | **Chiusa sul verde (BUILD rifinitura HIG — R-04).** **R-04 — etichette e tap target dei bottoni d'azione (trasversale)**: i bottoni in coda di riga («Fondi»/«Rimuovi») non dicevano QUALE elemento a VoiceOver e rischiavano tap target <44pt; la riga-candidato compressione avvolgeva l'id grezzo senza dire cosa fa il tocco. **Layer PURO (oracolo, target_tests)**: `ContactRow.actionLabel` («Fondi ‹nome›», riusa il fallback «Contatto senza nome»), `CalendarRow.actionLabel` («Rimuovi calendario ‹titolo›»), `CompressionCandidate.actionHint` («Stima il risparmio» — il tocco stima, non comprime). **View (compilate-non-rese)**: `accessibilityLabel` specifica azione+oggetto sui bottoni, `.frame(minHeight:44)`+`.contentShape(Rectangle())` per il tap target, `.accessibilityHint(actionHint)` sulla riga compressione; `swipeActions` distruttivo idiomatico su entrambe le righe extra-foto, instradato allo **STESSO** gate human-gated del bottone (nessuna scorciatoia distruttiva, T-092). **VERDE (comando, L-COL-002)**: **CI run #42 `success`** (`59616f0`, verde al primo colpo) — `swift build` (-warnings-as-errors), `swift test` (target_tests + regressione), `swiftlint lint --strict`, build app iOS. **Copertura (L-COL-006)**: i 3 helper puri coperti dai target_tests **incluse le contro-prove** (etichetta non isolata + oggetto presente; fallback contatto senza nome; hint dichiara la stima e non nomina la compressione); View/`swipeActions`/frame 44pt compilati dai due job CI ma **senza test di rendering** → resa VoiceOver e tap-target a runtime **dichiarati non coperti**. **Nessuna logica nuova Domain/Data**: altitudine invariata; baseline privacy invariata (nessun permesso/rete/framework nuovo). **Prossimo: R-05** (stati vuoto/errore uniformi su `ContentUnavailableView`). **[Storico R-03, chiuso a run #41 (`57e919a`)]:** raggruppamento VoiceOver di righe/card (trasversale): ogni riga/card esponeva 3-4 elementi VoiceOver separati (titolo, conteggio, byte, «stima» / oppure il `localIdentifier` PhotoKit grezzo letto ad alta voce). Ora ogni riga è **un solo elemento leggibile** con etichetta e valore **umani**. **Layer PURO (oracolo, target_tests)**: helper di accessibilità in `CategoryReviewPresentation.Row` (label «Elemento» + value disposizione, **mai l'id grezzo**), `Dashboard`/`HonestReportPresentation.CategoryRow` (label = titolo, `accessibilityValue(formattedBytes:)` con conteggio/byte/marca-stima, singolare coniugato), `CompressionCandidate` (label «Video» + value, **mai l'id grezzo**). **View (compilate-non-rese)**: righe categoria (Dashboard, HonestReport), righe review (CategoryReview) e righe candidato (Compression) → `accessibilityElement(children:.ignore)` + label/value dagli helper puri; NonGoal row → `children:.combine`; `isHeader` su hero/titoli di stato e header di sezione (Home, Dashboard, CategoryReview, HonestReport, NonGoals, Compression); `accessibilityHidden(true)` sulle SF Symbol decorative dei Label/card. Estratte le 4 sezioni di navigazione della Dashboard in `DashboardView+Sections.swift` (extension, come `CompressionView+Sections`) per rientrare in `type_body_length` (259→sotto 250) dopo le aggiunte di accessibilità; proprietà di `DashboardView` da `private` a `internal`. **VERDE (comando, L-COL-002)**: **CI run #41 `success`** (`57e919a`) — `swift build` (-warnings-as-errors), `swift test` (target_tests + regressione, 0 failures), `swiftlint lint --strict`, build app iOS. **NB (L-COL-003)**: run #40 (`8988efa`) era **ROSSO solo sul lint** (`type_body_length` su `DashboardView`, corpo a 259>250 dopo le aggiunte; build e test già verdi); fix = estrazione in `+Sections`, riverificata verde a #41. **Copertura (L-COL-006)**: helper puri coperti dai target_tests **incluse le contro-prove** («mai l'id grezzo» per CategoryReview/Compression; marca-stima e singolare per Dashboard/HonestReport); le View sono compilate dai due job CI ma **senza test di rendering** → la resa VoiceOver a runtime è **dichiarata non coperta**. **Nessuna logica nuova Domain/Data**: altitudine invariata. Baseline privacy invariata (nessun permesso/rete/framework nuovo; solo SF Symbols e API accessibilità di sistema). L'estrazione `+Sections` è riorganizzazione, **nessuna cancellazione di dead-code**. **(Poi chiuso: R-04 a run #42.)** **[Storico R-02, chiuso a run #39 (`091b71f`)]:** titolo unico per schermata — nav bar `.inline` guardata `#if canImport(UIKit)` su Home/Dashboard/CategoryReview/Compression/HonestReport, replicando `NonGoalsView`; ExtraPhotoDomains/ThemeSettings invariate (già conformi). Solo-View, nessun target_test nuovo. **[Storico R-00 + R-01, chiusi a run #38]:** Costruiti i due task **alti** della coda R-* del `HIG-REFINEMENT-PLAN.md`, ognuno chiuso al confine CI. **R-00 — persistenza onboarding (bug reale)**: `App/ContentView.swift` passava `didFinishOnboarding` come `@State` → azzerato a ogni cold-launch, l'onboarding-manifesto ricompariva a ogni avvio; ora `@AppStorage(OnboardingGate.didFinishStorageKey)`, compare una sola volta per installazione. Decisione «mostrare l'onboarding?» estratta nel PURO `OnboardingGate` (Features) → oracolo `OnboardingGateTests` (3 target_test) su Linux. **R-01 — cifra-hero scalabile (trasversale)**: le cifre-hero usavano una dimensione FISSA `.font(.system(size: 44/40 …))`, non scalabile col Dynamic Type e troncata alle AX sizes, con `monospacedDigit` incoerente. Introdotto `AuroraType` (PURO): stile cifra-hero come dati — text style semantico `largeTitle`, rounded, bold, `monospacedDigit`, cap Dynamic Type `accessibility3` (oracolo `AuroraTypeTests`, 3 target_test). Resa SwiftUI in `AuroraTheme`: `Font` derivato + modificatore `.auroraHeroNumber()` (`dynamicTypeSize` capped, `.contentTransition(.numericText())` **gated su Reduce Motion**, unico punto di traduzione). Riuso su tutte le cifre-hero: Dashboard, Review categorie, Report onesto (esatta+stima), Compressione; `monospacedDigit` aggiunto anche al valore «liberabili ora» del Report onesto. **VERDE (comando, L-COL-002)**: **CI run #38 `success`** — build (-warnings-as-errors), test (target_tests+regressione), `swiftlint lint --strict`, build app iOS. NB: run #37 era rosso **solo** sul lint (nesting `AuroraType.swift:23`, `TextStyle` a 2 livelli → promosso a `AuroraType.HeroTextStyle`), fix riverificata su #38. **Copertura (L-COL-006)**: helper puri coperti dai target_test (incl. cap Dynamic Type e cifre monospaziate); le View SwiftUI compilate dai due job CI ma **senza test di rendering**. **Nessuna logica nuova Domain/Data**: altitudine invariata. Baseline privacy invariata (nessun nuovo permesso/rete/framework). Prossimo: **R-02** (titolo unico per schermata, niente doppioni nav-large + header gradiente su 6 schermate). |
...
> **⭐ IN CORSO = BUILD della rifinitura HIG (5/12).** Coda `R-00…R-11` del
...
> **Prossimo consigliato: R-05** (stati vuoto/errore uniformi su
> `ContentUnavailableView`). Poi i restanti medi R-06…R-09, infine i bassi
> R-10/R-11. Nessuna logica
...
| Ultimo commit | `59616f0` feat(ui-shell) R-04 etichette azione+oggetto e tap target dei bottoni d'azione — **CI verde run #42** (verde al primo colpo). Precedente: `57e919a` (R-03 estrai sezioni Dashboard in `+Sections`, run #41), `8988efa` (R-03 raggruppamento VoiceOver — rosso solo sul lint a run #40, corretto da `57e919a`), `091b71f` (R-02, run #39), `f0b22ec`/`dea3fc5` (R-01, run #38), `f934ad4` (R-00) |
