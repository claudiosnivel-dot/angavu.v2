...
| Ultimo commit | `31b413b` **blueprint motore di scansione veloce** (`blueprint/FAST-SCAN-ENGINE-PLAN.md`, FSE-A…G, nessun codice) + stato sessione. Ultimo CODICE verificato: `3460690` **scansione unificata** (una barra 3 fasi + residenza P0-2b cablata + cache popolata a fine scan → Dashboard istantanea) — **CI run #78 `success`**. **Piano POST-DEVICE COMPLETO e verde** (P0/A/B/C/D/E/P0-2b, run #60→#75); scansione unificata #78. **Prossima sessione — DECISO dall'utente: implementare il blueprint FSE** (motore di scansione veloce, dal DAG, **FSE-A** per prima → poi ripensare la residenza, la fase più pesante). Consigliato prima/insieme: test on-device (runtime device non coperto dalla CI, L-COL-006). Merge su `main` **SOSPESO** (scelta utente). CI normale (repo pubblico). |
